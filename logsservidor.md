...
[*] Changed the `Cart` table
31 |   if (stderr.trim()) {
  [+] Added unique index on columns (sessionId, status)

[*] Changed the `Order` table
  [+] Added unique index on columns (sessionId, status)
  [+] Added unique index on columns (customerId, status)
32 |     console.error(stderr.trim());
33 |   }
34 | 
35 |   if (exitCode !== 0) {
36 |     const failure = new Error(errorMessage ?? `Command failed: ${command.join(" ")}`) as Error & {
                             ^
error: Operational schema validation failed.
 details: {
  exitCode: 1,
  stdout: "[*] Changed the `Cart` table\n  [+] Added unique index on columns (sessionId, status)\n\n[*] Changed the `Order` table\n  [+] Added unique index on columns (sessionId, status)\n  [+] Added unique index on columns (customerId, status)\n",
  stderr: "La base de datos no coincide con prisma/schema.prisma despues de migrate deploy. Aborto antes del seed para evitar errores operativos como P2022.\n",
},

      at <anonymous> (/app/prisma/bootstrap.ts:36:25)
...
✔ Generated Prisma Client (v6.19.2) to ./node_modules/@prisma/client in 166ms
...
✔ Generated Prisma Client (v6.19.2) to ./node_modules/@prisma/client in 150ms
